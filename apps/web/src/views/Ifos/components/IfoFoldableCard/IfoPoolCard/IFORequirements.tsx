...
const NotOkNFT = ({ admissionProfile }) => {
  ...
  const keyword = '%Diffusion Squad NFT%'

  const rawText = t(`Set %Diffusion Squad NFT% as Diffusion Profile avatar`)
...
const NotOkProfilePoints = ({ pointThreshold }) => {
  ...
  const keyword = '%Diffusion Profile%'

  const rawText = t(`Reach %point% or more %Diffusion Profile% points`, { point: pointThreshold })
